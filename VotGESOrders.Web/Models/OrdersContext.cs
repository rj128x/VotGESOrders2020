	...
	public class OrdersContext
	{
		...
		public IQueryable<Order> Orders {
			get {
				try {
					...
					Logger.info("Возврат списка заказов (по умолчанию) "+resultOrders.Count.ToString(), Logger.LoggerSource.ordersContext);
					...
					throw new Exception(String.Format("Ошибка при получении списка заказов (по умолчанию)"));
				}
			}
		}
		...
		public IQueryable<Order> OrdersActive {
			get {
				try {
					...
					throw new Exception(String.Format("Ошибка при получении списка заказов (активные)"));
				}
		...
		public IQueryable<Order> OrdersActiveExpired {
			get {
				try {
					...
					throw new Exception(String.Format("Ошибка при получении списка заказов (просроченных)"));
				}

			}
		}

        public IQueryable<Order> OrdersNotClosed
        {
            get
            {
                try
                {
                    Logger.info("Получение списка заказов", Logger.LoggerSource.ordersContext);
                    OrdersUser currentUser = new OrdersUser();
                    VotGESOrdersEntities context = new VotGESOrdersEntities();
                ...
                {
                    Logger.error("===Ошибка при получении списка заказов " + e.ToString(), Logger.LoggerSource.ordersContext);
                    throw new Exception(String.Format("Ошибка при получении списка заказов "));
                }
        ...
        public IQueryable<Order> getOrdersUserFilter(OrderFilter filter) {
			...
			try {
				...
				throw new Exception(String.Format("Ошибка при получении списка заказов (фильтр)"));
			}
		}
			...
			try {
				...
				if (!isNew) {
					...
					if (!tempOrder.AllowChangeOrder) {
						throw new Exception("Нельзя изменить заявку");
					}
				...
				if (order.OrderIsExtend) {
					...
					if (!parentOrder.AllowExtendOrder && isNew) {
						throw new Exception("Заявка уже продлена");
					}
					...
					if (isNew) {
						MailContext.sendMail(String.Format("Заявка №{0}. Продление заявки ({2}) [{1}]", parentOrderNew.OrderYearNumber.ToString("#.##", OrderInfo.Current.NFI),
				...
				if (order.OrderIsFixErrorEnter) {
					...
					if (isNew) {
						...
						MailContext.sendMail(String.Format("Заявка №{0}. Заявка закрыта без ввода оборудования ({2}) [{1}]",
							parentOrderNew.OrderYearNumber.ToString("#.##", OrderInfo.Current.NFI), parentOrderNew.FullOrderObjectInfo, CurrentUser.FullName),
							parentOrderNew, false, false);
					}
				}
				...
				MailContext.sendMail(String.Format(isNew ? "Заявка №{0}. Создана новая заявка ({2}) [{1}]" : "Заявка №{0}. Заявка изменена ({2}) [{1}]", 
					order.OrderYearNumber.ToString("#.##", OrderInfo.Current.NFI), order.FullOrderObjectInfo, CurrentUser.FullName), 
				...
				throw new Exception("Ошибка при создании/изменении заявки");
			}
		}

		public void ChangeOrder(Order order, Guid guid) {
			Logger.info("Пользователь изменил заявку №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);
			try {
				RegisterOrder(order, guid);
			} catch (Exception e) {
				Logger.error(String.Format("===Ошибка при изменении заявки №{1}: {0}", e, order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)), Logger.LoggerSource.ordersContext);
				if (e is Exception) {
					throw e;
				}
				throw new Exception(String.Format("Ошибка при изменении заявки №{0}", order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)));
			}
		}


		public void AcceptOrder(Order order, Guid guid) {
			Logger.info("Пользователь разрешил заявку №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);
			try {
				VotGESOrdersEntities context=new VotGESOrdersEntities();
				...
				if (order.AllowReviewOrder) {
					...
					context.SaveChanges();
					LastUpdate.save(guid);
					Logger.info("===Заявка разрешена. Заявка №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);
				} else {
					throw new Exception("Нельзя разрешить заявку");
				}
				order.refreshOrderFromDB(orderDB, currentUser, false, null);
				MailContext.sendMail(String.Format("Заявка №{0}. Заявка разрешена ({2}) [{1}]", 
					orderDB.orderYearNumber.Value.ToString("#.##", OrderInfo.Current.NFI), order.FullOrderObjectInfo, CurrentUser.FullName), 
					order, false, false);
			} catch (Exception e) {
				Logger.error(String.Format("===Ошибка при разрешении заявки №{1}: {0}", e, order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)), Logger.LoggerSource.ordersContext);
				if (e is Exception) {
					throw e;
				}
				throw new Exception(String.Format("Ошибка при разрешении заявки №{0}", order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)));
			}
		}

		public void BanOrder(Order order, Guid guid) {
			Logger.info("Пользователь запретил заявку  №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);
			try {
				VotGESOrdersEntities context=new VotGESOrdersEntities();
				Orders orderDB=context.Orders.First(o => o.orderNumber == order.OrderNumber);
				order.checkPremissions(orderDB, currentUser);
				if (order.AllowReviewOrder) {
					...
					context.SaveChanges();
					LastUpdate.save(guid);
					Logger.info("===Заявка запрещена. Заявка №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);

				} else {
					throw new Exception("Нельзя отклонить заявку");
				}
				order.refreshOrderFromDB(orderDB, currentUser, false, null);
				...
				Logger.error(String.Format("===Ошибка при запрещении заявки №{1}: {0}", e, order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)), Logger.LoggerSource.ordersContext);
				if (e is Exception) {
					throw e;
				}
				throw new Exception(String.Format("Ошибка при запрете заявки №{0}", order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)));
			}
		}


		public void RejectReviewOrder(Order order, Guid guid) {
			Logger.info("Пользователь отменил рассмотрение заявки  №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);
			try {
				VotGESOrdersEntities context=new VotGESOrdersEntities();
				Orders orderDB=context.Orders.First(o => o.orderNumber == order.OrderNumber);
				order.checkPremissions(orderDB, currentUser);
				if (order.AllowRejectReviewOrder) {
					...
					context.SaveChanges();
					LastUpdate.save(guid);
					Logger.info("===Заявка. Отмена рассмотрения. Заявка №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);

				} else {
					throw new Exception("Нельзя отменить рассмотрение заявки");
				}
				order.refreshOrderFromDB(orderDB, currentUser, false, null);
				...
				Logger.error(String.Format("===Ошибка при отмене рассмотрения заявки №{1}: {0}", e, order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)), Logger.LoggerSource.ordersContext);
				if (e is Exception) {
					throw e;
				}
				throw new Exception(String.Format("Ошибка при отмене рассмотрения заявки №{0}", order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)));
			}
		}


		public void OpenOrder(Order order, Guid guid) {
			Logger.info("Пользователь открыл заявку  №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);
			try {
				VotGESOrdersEntities context=new VotGESOrdersEntities();
				Orders orderDB=context.Orders.First(o => o.orderNumber == order.OrderNumber);
				order.checkPremissions(orderDB, currentUser);
				if (order.AllowOpenOrder) {
					...
					context.SaveChanges();
					LastUpdate.save(guid);
					Logger.info("===Заявка открыта. Заявка №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);
				} else {
					throw new Exception("Нельзя открыть заявку");
				}
				order.refreshOrderFromDB(orderDB, currentUser, false, null);
				...
				Logger.error(String.Format("===Ошибка при открытии заявки №{1}: {0}", e, order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)), Logger.LoggerSource.ordersContext);
				if (e is Exception) {
					throw e;
				}
				throw new Exception(String.Format("Ошибка при открытии заявки №{0}", order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)));
			}
		}

		public void RejectOpenOrder(Order order, Guid guid) {
			Logger.info("Пользователь отменил открытие заявки  №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);
			try {
				VotGESOrdersEntities context=new VotGESOrdersEntities();
				Orders orderDB=context.Orders.First(o => o.orderNumber == order.OrderNumber);
				order.checkPremissions(orderDB, currentUser);
				if (order.AllowRejectOpenOrder) {
					...
					context.SaveChanges();
					LastUpdate.save(guid);
					Logger.info("===Заявка. отмена открытия. Заявка №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);
				} else {
					throw new Exception("Нельзя отменить открытие заявки");
				}
				order.refreshOrderFromDB(orderDB, currentUser, false, null);
				...
				Logger.error(String.Format("===Ошибка при отмене открытия заявки №{1}: {0}", e, order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)), Logger.LoggerSource.ordersContext);
				if (e is Exception) {
					throw e;
				}
				throw new Exception(String.Format("Ошибка при отмене открытия заявки №{0}", order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)));
			}
		}

		public void CloseOrder(Order order, Guid guid) {
			Logger.info("Пользователь разрешил ввод оборудования. Заявка  №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);
			try {
				VotGESOrdersEntities context=new VotGESOrdersEntities();
				Orders orderDB=context.Orders.First(o => o.orderNumber == order.OrderNumber);
				order.checkPremissions(orderDB, currentUser);
				if (order.AllowCloseOrder) {
					...
					context.SaveChanges();
					LastUpdate.save(guid);
					Logger.info("===Разрешен ввод оборудования. Заявка №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);
				} else {
					throw new Exception("Нельзя разрешить ввод оборудования");
				}
				order.refreshOrderFromDB(orderDB, currentUser, false, null);
				...
				Logger.error(String.Format("===Ошибка при разрешении ввода. Заявка №{1}: {0}", e, order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)), Logger.LoggerSource.ordersContext);
				if (e is Exception) {
					throw e;
				}
				throw new Exception(String.Format("Ошибка при разрешении ввода. Заявка №{0}", order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)));
			}
		}

		public void RejectCloseOrder(Order order, Guid guid) {
			Logger.info("Пользователь отменил разрешение на ввод оборудования. Заявка  №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);
			try {
				VotGESOrdersEntities context=new VotGESOrdersEntities();
				Orders orderDB=context.Orders.First(o => o.orderNumber == order.OrderNumber);
				order.checkPremissions(orderDB, currentUser);
				if (order.AllowRejectCloseOrder) {
					...
					context.SaveChanges();
					LastUpdate.save(guid);
					Logger.info("===Отменено разрешение на ввод оборудования. Заявка №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);
				} else {
					throw new Exception("Нельзя отменить разрешение на ввод оборудования");
				}
				order.refreshOrderFromDB(orderDB, currentUser, false, null);
				...
				Logger.error(String.Format("===Ошибка при отмне разрешения на ввода. Заявка №{1}: {0}", e, order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)), Logger.LoggerSource.ordersContext);
				if (e is Exception) {
					throw e;
				}
				throw new Exception(String.Format("Ошибка при отмене разрешения на ввод. Заявка №{0}", order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)));
			}
		}

		public void CancelOrder(Order order, Guid guid) {
			Logger.info("Пользователь снял заявку №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);
			try {
				VotGESOrdersEntities context=new VotGESOrdersEntities();
				Orders orderDB=context.Orders.First(o => o.orderNumber == order.OrderNumber);
				order.checkPremissions(orderDB, currentUser);
				if (order.AllowCancelOrder) {
					...
					context.SaveChanges();
					LastUpdate.save(guid);
					Logger.info("===Заявка. Снята заявка №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);
				} else {
					throw new Exception("Нельзя снять заявку");
				}
				order.refreshOrderFromDB(orderDB, currentUser, false, null);
				...
				Logger.error(String.Format("===Ошибка при снятии заявки №{1}: {0}", e, order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)), Logger.LoggerSource.ordersContext);
				if (e is Exception) {
					throw e;
				}
				throw new Exception(String.Format("Ошибка при снятии заявки №{0}", order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)));
			}
		}

		public void RejectCancelOrder(Order order, Guid guid) {
			Logger.info("Пользователь отменил снятие заявки №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);
			try {
				VotGESOrdersEntities context=new VotGESOrdersEntities();
				Orders orderDB=context.Orders.First(o => o.orderNumber == order.OrderNumber);
				order.checkPremissions(orderDB, currentUser);
				if (order.AllowRejectCancelOrder) {
					...
					context.SaveChanges();
					LastUpdate.save(guid);
					Logger.info("===Заявка. Отмена снятия. Заявка №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);
				} else {
					throw new Exception("Нельзя отменить снятие заявки");
				}
				order.refreshOrderFromDB(orderDB, currentUser, false, null);
				...
				Logger.error(String.Format("===Ошибка при отмене снятия заявки №{1}: {0}", e, order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)), Logger.LoggerSource.ordersContext);
				if (e is Exception) {
					throw e;
				}
				throw new Exception(String.Format("Ошибка при отмене снятия заявки №{0}", order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)));
			}
		}

		public void CompleteOrder(Order order, Guid guid) {
			Logger.info("Пользователь ввел оборудование. Заявка №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);
			try {
				VotGESOrdersEntities context=new VotGESOrdersEntities();
				Orders orderDB=context.Orders.First(o => o.orderNumber == order.OrderNumber);
				order.checkPremissions(orderDB, currentUser);
				if (order.AllowCompleteOrder) {
					...
					context.SaveChanges();
					LastUpdate.save(guid);
					Logger.info("===Оборудование введено. Заявка №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);
				} else {
					throw new Exception("Нельзя закрыть заявку");
				}
				order.refreshOrderFromDB(orderDB, currentUser, false, null);
				...
				Logger.error(String.Format("===Ошибка при вводе оборудования №{1}: {0}", e, order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)), Logger.LoggerSource.ordersContext);
				if (e is Exception) {
					throw e;
				}
				throw new Exception(String.Format("Ошибка при вводе оборудования по заявке №{0}", order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)));
			}
		}

		public void RejectCompleteOrder(Order order, Guid guid) {
			Logger.info("Пользователь отменил ввод оборудования. Заявка №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);
			try {
				VotGESOrdersEntities context=new VotGESOrdersEntities();
				Orders orderDB=context.Orders.First(o => o.orderNumber == order.OrderNumber);
				order.checkPremissions(orderDB, currentUser);
				if (order.AllowRejectCompleteOrder) {
					...
					context.SaveChanges();
					LastUpdate.save(guid);
					Logger.info("===Отмена ввода оборудования. Заявка №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);
				} else {
					throw new Exception("Нельзя отменить ввод оборудования");
				}
				order.refreshOrderFromDB(orderDB, currentUser, false, null);
				...
				Logger.error(String.Format("===Ошибка при отмене ввода оборудования по заявке №{1}: {0}", e, order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)), Logger.LoggerSource.ordersContext);
				if (e is Exception) {
					throw e;
				}
				throw new Exception(String.Format("Ошибка при отмене ввода оборудования по заявке №{0}", order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)));
			}
		}

		public void RegisterEditOrder(Order order, Guid guid) {
			Logger.info("Пользователь отредактировал заявку. Заявка №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);
			try {
				...
				if (order.AllowEditOrder) {
					...
					context.SaveChanges();
					LastUpdate.save(guid);
					Logger.info("===Заявка отредактирована. Заявка №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);
				} else {
					throw new Exception("Нельзя отменить ввод оборудования");
				}
				order.refreshOrderFromDB(orderDB, currentUser, false, null);
				...
				Logger.error(String.Format("===Ошибка при редактировании заявки №{1}: {0}", e, order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)), Logger.LoggerSource.ordersContext);
				if (e is Exception) {
					throw e;
				}
				throw new Exception(String.Format("Ошибка при редактировании заявки №{0}", order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)));
			}
		}

		public void RegisterAddComment(Order order, String commentText, Guid guid) {
			Logger.info("Пользователь добавил комментарий к заявке. Заявка №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);
			try {
				VotGESOrdersEntities context=new VotGESOrdersEntities();
				Orders orderDB=context.Orders.First(o => o.orderNumber == order.OrderNumber);
				order.checkPremissions(orderDB, currentUser);
				...
				context.SaveChanges();
				LastUpdate.save(guid);
				Logger.info("===Добавлен комментарий. Заявка №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);
				...
				Logger.error(String.Format("===Ошибка при комментировании заявки №{1}: {0}", e, order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)), Logger.LoggerSource.ordersContext);
				if (e is Exception) {
					throw e;
				}
				throw new Exception(String.Format("Ошибка при комментировании заявки №{0}", order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)));
			}
		}
		...
		public void ReloadOrder(Order order) {
			Logger.info("Пользователь обновляет заявку №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);
			try {
				VotGESOrdersEntities context=new VotGESOrdersEntities();
				Orders orderDB=context.Orders.First(o => o.orderNumber == order.OrderNumber);
				Logger.info("===Заявка обновлена. Заявка №" + order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI), Logger.LoggerSource.ordersContext);
				order.refreshOrderFromDB(orderDB, currentUser, false, null);
			} catch (Exception e) {
				Logger.error(String.Format("===Ошибка при обновлении заявки №{1}: {0}", e, order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)), Logger.LoggerSource.ordersContext);
				throw new Exception(String.Format("Ошибка при обновлении заявки №{0}", order.OrderNumber.ToString("#.##", OrderInfo.Current.NFI)));
			}
		}
